-- Merge duplicate chapters whose absolute URLs only differ by host/domain.
-- Example:
--   https://sayhentai.vin/doc-truyen/a/chapter-1.html
--   https://sayhentai.uno/doc-truyen/a/chapter-1.html

BEGIN;

DROP TABLE IF EXISTS tmp_duplicate_chapter_path_map;

CREATE TEMP TABLE tmp_duplicate_chapter_path_map AS
WITH ranked AS (
  SELECT
    c.id,
    c.manga_title_id,
    regexp_replace(c.href, '^https?://[^/]+', '', 'i') AS href_key,
    FIRST_VALUE(c.id) OVER (
      PARTITION BY c.manga_title_id, regexp_replace(c.href, '^https?://[^/]+', '', 'i')
      ORDER BY
        COUNT(i.id) DESC,
...
FROM tmp_duplicate_chapter_path_map m
...
USING tmp_duplicate_chapter_path_map m
